...
/**
 * Tunables for hand mouse control (not stored in CONSTANT.kt).
 * Pinch/fist distances are normalized by hand scale (wrist→middle-MCP length).
 */
data class HandMouseConfig(
    val controlScheme: ControlScheme = ControlScheme.HYBRID_IMU_HAND,
    ...
    companion object {
        val SolutionA = HandMouseConfig(controlScheme = ControlScheme.TOUCH_GATED_TRACKPAD)
        val SolutionB = HandMouseConfig(controlScheme = ControlScheme.IMU_HEAD_POINTER)
        val SolutionC = HandMouseConfig(
            controlScheme = ControlScheme.HYBRID_IMU_HAND,
            usePalmCenter = false,
            moveDeadzone = 0.001f,
            sensitivity = 1500f,
        )
        val Default = SolutionC
        val LegacyTranslation = HandMouseConfig(controlScheme = ControlScheme.TRANSLATION_PINCH)
        val LegacyPointingFist = HandMouseConfig(controlScheme = ControlScheme.POINTING_FIST)
    }
}
