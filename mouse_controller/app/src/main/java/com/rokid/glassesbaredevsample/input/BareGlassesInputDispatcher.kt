...
class BareGlassesInputDispatcher(context: Context) {
    ...
    /** TouchPad 单指长按抬起 · `KEYCODE_PROG_BLUE` up → [BareKeyEvent.LongPressRelease] */
    fun dispatchLongKeyRelease() {
        notifyIntercept("Key·PROG_BLUE·UP", consumed = true)
        handler?.invoke(BareKeyEvent.LongPressRelease)
    }

    /** TouchPad 单指前/后滑 · `KEYCODE_DPAD_*` → [BareKeyEvent.SwipeForward] / [SwipeBack] */
    fun dispatchSwipe(event: BareKeyEvent) {
        val label = when (event) {
            BareKeyEvent.SwipeForward -> "Key·DPAD·前滑"
            BareKeyEvent.SwipeBack -> "Key·DPAD·后滑"
            else -> return
        }
    ...
    /** RG TouchPad two-finger single tap · `KEYCODE_NOTIFICATION` → [BareKeyEvent.TwoFingerSingleTap] */
    fun dispatchTwoFingerTap() {
        notifyIntercept("Key·NOTIFICATION·双指单击", consumed = true)
        handler?.invoke(BareKeyEvent.TwoFingerSingleTap)
    }
    ...
    companion object {
        ...
        private fun actionToEvent(action: String): BareKeyEvent? = when (action) {
            ...
            KeyEventAction.TWO_FINGER_SINGLE.action -> BareKeyEvent.TwoFingerSingleTap
            else -> null
        }
