...
object MouseLinkPacket {
    ...
    const val FLAG_RECENTER: Int = 1 shl 3
    const val FLAG_MOTION_PAUSED: Int = 1 shl 4
    const val FLAG_CENTER_CURSOR: Int = 1 shl 5
    const val FLAG_PRECISION_MODE: Int = 1 shl 6
    ...
    data class Decoded(
        ...
        val recenter: Boolean get() = flags and FLAG_RECENTER != 0
        val motionPaused: Boolean get() = flags and FLAG_MOTION_PAUSED != 0
        val centerCursor: Boolean get() = flags and FLAG_CENTER_CURSOR != 0
        val precisionMode: Boolean get() = flags and FLAG_PRECISION_MODE != 0
    ...
    fun encode(
        ...
        val flags = buildFlags(
            ...
            recenter = command.recenter,
            motionPaused = command.motionPaused,
            centerCursor = command.centerCursor,
            precisionMode = command.precisionMode,
        )
    ...
    fun buildFlags(
        ...
        if (recenter) flags = flags or FLAG_RECENTER
        if (motionPaused) flags = flags or FLAG_MOTION_PAUSED
        if (centerCursor) flags = flags or FLAG_CENTER_CURSOR
        if (precisionMode) flags = flags or FLAG_PRECISION_MODE
        return flags
    }
